...
class pyValveApp(tk.Frame):
    def __init__(self, parent):
        ...
        serPortVar = tk.StringVar(self.parent)
        serPortList = [port.device for port in serialCom.getPorts()]

        if(serPortList):
            serPortVar.set(serPortList[0])
        selSerPort = tk.OptionMenu(self.parent, serPortVar, *serPortList)
        selSerPort.place(x=420, y=70)

        buttonOpenSerial = tk.Button(text="Konektuj inferfejs", command=lambda: serialCom.open(self, self.parent, serPortVar.get()))


        buttonOpenSerial.place(x=470, y=100)
...
class serialCom():
    ...
    def open(self, parent, port):
        ...
        if(ser.inWaiting()>0):
            print(ser.readline())
            self.parent.buttonOpenSerial.config(bg="GREEN")
            return True
        return FALSE
